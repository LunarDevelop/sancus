...
class GIFS(commands.Cog):
    ...
    async def gif(self, ctx, search_term):
        ...
        url = self.RANDOMURL + "key=%s&q=%s&media_filter=%s&limit=%s&contentfilter=%s" % (
            self.APIKEY, search_term, "minimal", 10, "medium")
        ...
        if r.ok:
            try:
                ...
                await ctx.send("I can't find any gif matching that word")
