            ...
            if "_embeds" in section and section != "USERS" and "_timeouts" not in section:
                ...
                for option in self.config_.options(section):
                    if self.EMBEDS['DEFAULT'].get(option) != None:
                        self.EMBEDS[section[:-7]][option] = self.config_.get(section, option)
                ...
                for option in self.config_.options(section):
                    if self.TIMEOUTS['DEFAULT'].get(option) != None:
                        self.TIMEOUTS[section[:-9]][option] = self.config_.get(section, option)
    ...
    def save(self):
        ...
        for section in self.EMBEDS:
            if section != "DEFAULT":
                for option in self.EMBEDS[section]:
                    try:
                        self.config_.set(f"{section}_embeds", str(option), str(self.EMBEDS[section][option]))
        ...
        for section in self.TIMEOUTS:
            if section != "DEFAULT":
                for option in self.TIMEOUTS[section]:
                    try:
                        self.config_.set(f"{section}_timeouts", str(option), str(self.TIMEOUTS[section][option]))
        ...
        for section in self.USERS:
            ...
            for key in self.USERS[section]:
                try:
                    self.USERFile.set(str(section), key, str(self.USERS[section].get(key)))
                except:
                    self.USERFile.add_section(str(section))
                    self.USERFile.set(section, key, str(self.USERS[section].get(key)))
